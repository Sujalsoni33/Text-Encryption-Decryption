...
import Hybrid.HybridDecryptMessageScreen as HybridDecryptMessageScreen
...
def decipher_screen(root):
    ...
    def message_validation(message):
        if len(message) <= 0:
            ...
            messagebox.showerror("MESSAGE ERROR", "Ciphertext must contain digits only!")
            return False
        return True
    ...
    def decipher_message():
        if messagebox.askquestion("ATTENTION", "are you sure you entered the correct key?") == "yes":
            ...
            if key_validation(key, message) and message_validation(message):
                decipher_frame.destroy()
                HybridDecryptMessageScreen.deciphered_screen(root, key=key, message=message)
